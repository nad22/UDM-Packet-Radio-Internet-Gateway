...
// Arduino Core - MUSS als erstes include sein!
#include <Arduino.h>
...
#include <esp_system.h>    // Für esp_reset_reason_t
#include <driver/ledc.h>   // Für LEDC-Funktionen
...
void playPacketAudio(const String& hexData, bool isTx); // Play packet radio audio
void playI2SSamples(int16_t* samples, size_t sampleCount); // Play audio samples
void playAFSKToneI2S(int frequency, int durationUs, bool isTx); // Play AFSK tone
void playAFSKBit(bool bit, bool isTx);         // Play single AFSK bit
void playByte(uint8_t data, bool isTx);        // Play byte as audio
void playTone(int frequency, int durationUs);  // Play tone
void playBit(bool bit, bool isTx);             // Play bit
...
void setup() {
  ...
  // **KOMPATIBLE VERSION: Hardware Watchdog aktivieren**
  bootPrint("Init Watchdog...");
  esp_task_wdt_deinit(); // Eventuellen alten Watchdog zurücksetzen
  
  // Verwende die ältere API für ESP32 Core 2.x Kompatibilität
...
void setHardwareGain(uint8_t gainLevel) {
  ...
  // First detach any previous LEDC setup (ESP32 Core 2.x kompatibel)
  ledcDetachPin(I2S_GAIN_PIN);
